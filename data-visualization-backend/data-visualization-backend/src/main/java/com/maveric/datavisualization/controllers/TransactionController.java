...
public class TransactionController {
    ...
    @GetMapping("/getAllBanks")
  	public ResponseEntity<List<Bank>> getAllBanks() throws JsonProcessingException
  	{
    	logger.info("TransactionController-getAllBanks() call started");
      	List<Bank> getAllBankData =transactionService.getAllBanks();
      	logger.info("TransactionController-getAllBanks() "+getAllBankData);
  		if(getAllBankData==null)
  		{
  			logger.info("TransactionController-getAllBanks() data not present ");
  			return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
  		}
  		logger.info("TransactionController-getAllBanks() method ended ");
  		return ResponseEntity.status(HttpStatus.OK).body(getAllBankData);
  	}
    ...
    @GetMapping("/getAllTxns")
  	public ResponseEntity<List<Transaction>> getAllTxns() throws JsonProcessingException
  	{
    	logger.info("TransactionController-getAllTxns() call started");
  		...
  		return ResponseEntity.status(HttpStatus.OK).body(getAllTxnData);
  	}
      /**
       * 
       * @param id
    ...
    public ResponseEntity<TransactionDetails> createTransaction(@RequestBody @Valid TransactionDetails transactionDetails,
        ...
        logger.info("TransactionController-createTransaction() call started");
        logger.info("Transaction Details : {}, Uri : {},", transactionDetails, request.getRequestURI());
    ...
    public ResponseEntity<List<Transaction>> getPanStatus(@PathVariable Long ackNo)
    {
    	List<Transaction> getAllDataOnAckNo =transactionService.getTransactionsByackNo(ackNo);
    	
		return ResponseEntity.status(HttpStatus.CREATED).body(getAllDataOnAckNo);
