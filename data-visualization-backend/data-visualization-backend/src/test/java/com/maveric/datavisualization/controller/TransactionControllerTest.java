...
public class TransactionControllerTest {
    ...
    public void testGetByUserID() {
        ...
        txn1.setUserId(1L);
        userTransactions.add(txn1);
        when(transactionService.getTransactionsByUserID(userId)).thenReturn(userTransactions);
        ResponseEntity<List<Transaction>> response = transactionController.getByUserID(userId);
        assertEquals(HttpStatus.CREATED, response.getStatusCode());
        assertEquals(userTransactions, response.getBody());
        verify(transactionService, times(1)).getTransactionsByUserID(userId);
    }

    @Test
    public void testGetByUserIDNoTransactionsFound() {
        Long userId = 456L;
        List<Transaction> userTransactions = new ArrayList<>();
        when(transactionService.getTransactionsByUserID(userId)).thenReturn(userTransactions);
        ResponseEntity<List<Transaction>> response = transactionController.getByUserID(userId);
